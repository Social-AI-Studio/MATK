...
from datamodules.collators.intmeme import intmeme_collate_fn
...
class IntMemeDataModule(pl.LightningDataModule):
    def __init__(
        self,
        dataset_cfg: str,
        vla_processor_class_or_path: str,
        ...
        vla_processor = AutoProcessor.from_pretrained(vla_processor_class_or_path)
        mie_tokenizer = AutoTokenizer.from_pretrained(mie_tokenizer_class_or_path)
        self.collate_fn = partial(
            intmeme_collate_fn,
            labels=labels, 
            processor=vla_processor,
            tokenizer=mie_tokenizer
        )
